...
assessment process. Similar work in @Wang2021 but with a longest fishery history.
...
destra <- ggplot(mdat , 
                 aes(x=Year_yyyy, 
                     y=as.double(mdat$Density_gm2),
                     color=TS_Id_Method)) +
  geom_point() +
  geom_smooth(method = "loess", colour='red', fill='red', alpha=.3)+
  scale_x_continuous(breaks = seq(from = 190, to = 2020, by = 4))+
