...
````{r}
filepath <- "~/DOCAS/Data/Biomass_Krill_Trend"
...
den48 <- mdat %>% 
  ...
  summarize(denmean=(mean(as.double(Density_gm2)))) %>% 
  summarize(cvdenmean=(mean(as.double(CV_of_density_Perc))))
kable(den481) 
...
knitr::kable(mdat[mdat$CV_of_density_Perc=="95%CI 0-6.42"|mdat$CV_of_density_Perc=="95%CI 0.21-11.29", 1:10])

# The above do not have CV values we can work with so removing them from analysis
remdat <- rbind(remdat,  mdat[mdat$CV_of_density_Perc %in% c("95%CI 0-6.42", "95%CI 0.21-11.29")])
mdat <- mdat[!mdat$CV_of_density_Perc %in% c("95%CI 0-6.42", "95%CI 0.21-11.29")]
...
NoAreaTable <- mdat[mdat$Survey_area_km2 %in% c("")] # the "0" is the 2019 synoptic so retaining in the main data set

knitr::kable(NoAreaTable[, c(1:8,10)], caption = "No area available in Metadata - Required for CV!")

# Adding to the removed data table
remdat <-  rbind(remdat,  mdat[mdat$Survey_area_km2 %in% c("")])
